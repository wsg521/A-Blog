	  		...
	  		<div class="row">
	            ...
	            <div class="col-md-9">
					<div class="panel panel-default">
		              	<div class="panel-heading">Blog首页管理<font color="red;">（注：排序号为升序排列，md号位div列宽度：1~12个值）</font></div>    
		            	<div class="panel-body">
		            		<form id="updateBlog_form" onsubmit="return updateBlog()" method="post">
				                <div class="" id="title_div">
			              			<input type="text" name="home.title" value="${home.title}" class="form-control" placeholder="列表标题" onkeyup="javascript:$('#title_div').attr('class', '');" /> 
			              		</div> 
			              		<label></label>
			              		<input type="number" name="home.md" value="${home.md}" class="form-control stepper-input" placeholder="列表md号" title="列表md号" /> 
			              		<label></label>
			              		<input type="number" name="home.sequence" value="${home.sequence}" class="form-control stepper-input" placeholder="列表排序号" title="列表排序号" /> 
			              		<label></label>
			              		...
			              		<select name="home.ids" data-placeholder="请选择显示的Blog" class="chzn-select" style="width: 100px;" multiple="multiple" tabindex="4" />
									...
									<script type="text/javascript">
										var tags = '${home.ids}'.split(", ");
										for (var i = 0;i < tags.length;i++) {
											if(document.getElementById("blog"+tags[i]) != null)
												document.getElementById("blog"+tags[i]).selected="selected";
										}
									</script> 
								</select>
								<script type="text/javascript">$(".chzn-select").chosen();</script>	
								<br/><br/>
								<input type="text" name="home.id" value="${home.id}" style="display: none;" /> 
				              	<button type="submit" class="btn btn-info btn-block">修改Blog首页</button>
			              	</form>
                    	</div>	
                    </div>	    
	            </div>
